...
const START_TIME = process.env.START_TIME || "09:00"; // Утро — каждый день
const END_TIME   = process.env.END_TIME   || "18:00"; // Вечер — каждый день
...
/* ========= РОЛИ / ИСПОЛНИТЕЛИ ========= */
const INITIAL_ADMIN_IDS = ["549405058"]; // правьте под себя
...
/* ===== Персональные ежедневные списки (поправьте под себя) ===== */
...
/** "*" — ежедневно; можно добавить 1..7 (ISO: 1=Пн … 7=Вс) для особых дней */
...
async function ensureDailyTemplatesForDay(uid, weekdayISO) {
  ...
  const created = await addTasks({ creatorId: "system", assigneeId: uid, date: todayYMD(), titles: toCreate });
  return created;
}
...
bot.start(async (ctx) => {
  const hi =
    ...
    `Полезно: /inbox, /status (админ), /export, /myid, /daily_now (админ).`;
...
async function makePDF(rows, period, outDir) {
  ...
  await new Promise((resolve, reject) => {
    ...
    // Заголовок
    doc.fontSize(16).fillColor("#000").text(`Отчёт по задачам (${period})`, { underline: true });
    doc.moveDown(0.6);
...
bot.command("daily_now", async (ctx) => {
  ...
  for (const ex of EXECUTORS) {
    ...
    if (created?.length) {
      ...
      } catch (e) { console.warn("daily_now DM failed:", ex.id, e.message); }
    }
  }
...
async function runMorning(){
  const wd = jsWeekdayISO(new Date()); // 1..7
  let total = 0;
  try {
    for (const ex of EXECUTORS) {
      const created = await ensureDailyTemplatesForDay(String(ex.id), wd);
  ...
  console.log(`[morning] created=${total}`);
}
...
cron.schedule(timeToCron(END_TIME,"1-7"), async ()=>{
  try {
    ...
    if (GROUP_SUMMARY_CHAT_ID) {
      const stats = await statsForDate(ymd(today));
      const keys = Object.keys(stats);
      let msg = `📊 Сводка за ${ymd(today)}:\n`;
...
/* ========= START ========= */
// ⚠️ На старте прогоняем утреннюю логику один раз (антидубли встроены)
(async ()=>{
  ...
  // Автопрогон на старте (чтобы ежедневки пришли даже если бот запущен ПОСЛЕ START_TIME)
  await runMorning();
})();
